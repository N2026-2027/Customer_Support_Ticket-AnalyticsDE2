...
select
    -- 1. ID único (Surrogate Key) para que funcionen los conteos en el Dashboard
    ...
    -- 2. Lógica de SLA (Mapeo de 0/1 a texto para las leyendas de los gráficos)
    case 
        when f.sla_breached = 1 then 'SLA_Incumplido' 
        else 'SLA_Cumplido' 
    end as sla_status,

    -- 3. Todas las métricas confirmadas (satisfaction, resolution_time, etc.)
    f.*,
    ...
    -- 5. Nombre del mes (Necesario para los filtros y ejes X de Streamlit)
    case d.month
